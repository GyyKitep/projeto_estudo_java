...
public interface PedidoRepository extends JpaRepository<Pedido, Long>{
	...
	@Query("SELECT p FROM pedidos p JOIN p.user u WHERE u.username = :username")
	List<Pedido> findAllByUsuario(@Param("username")String username);
	
	@Query("SELECT p FROM pedidos p JOIN p.user u WHERE u.username = :username and p.status = :status")
